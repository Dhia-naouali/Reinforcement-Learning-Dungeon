...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor.act, params["batch_size"], params["ep_len"], render=log_video)
            ...
            if params['do_dagger']:
                ...
                for path in paths:
                    dagger_ob = path["observation"]
                    dagger_ac = expert_policy.get_action(dagger_ob)
                    path["action"] = dagger_ac
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          perm = np.random.permutation(len(replay_buffer))
          b = params["train_batch_size"]
          ob_batch = ptu.from_numpy(replay_buffer.obs[perm][:b])
          ac_batch = ptu.from_numpy(replay_buffer.acs[perm][:b])
        ...
        if log_video:
            ...
            eval_video_paths = utils.sample_n_trajectories(
                env, actor.act, MAX_NVIDEO, MAX_VIDEO_LEN, True)
        ...
        if log_metrics:
            ...
            eval_paths, eval_envsteps_this_batch = utils.sample_trajectories(
                env, actor.act, params['eval_batch_size'], params['ep_len'])
